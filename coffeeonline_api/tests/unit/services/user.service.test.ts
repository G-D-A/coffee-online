...
describe('UserService', () => {
    ...
    describe('getUserProfile', () => {
        it('should return user if found', async () => {
            const selectMock = jest.fn().mockResolvedValue({ id: 'user123', email: 'test@example.com' });
            (UserModel.findById as any).mockReturnValue({ select: selectMock });

            const result = await service.getUserProfile('user123');
            expect((UserModel.findById as any)).toHaveBeenCalledWith('user123');
